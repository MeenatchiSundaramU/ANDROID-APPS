...
class Maps2Activity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        mMap.addMarker(
            MarkerOptions().position(farmer).title("Farmer Location").icon(
            BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_GREEN)
        ))
        mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(farmer,10f))
        mMap.addMarker(
            MarkerOptions().position(LatLng(8.4068,77.6220)).title("Farmer Location").icon(
                BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_GREEN)
            ))
        mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(LatLng(8.4068,77.6220),10f))
        val industry= LatLng(getting!!.lat2, getting!!.long2)
        mMap.addMarker(
            MarkerOptions().position(industry).title("Industry Location").icon(
